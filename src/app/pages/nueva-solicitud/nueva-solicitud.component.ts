...
export class NuevaSolicitudComponent implements OnInit {
  ...
  constructor(private solicitudService: SolicitudService, private userService: UserService, private router: Router
  ...
  CrearSolicitud(): void{
    if(this.Validacion()){
      if (this.solicitudUsuario.usuario.Id === undefined){
        ...
        this.solicitudUsuario.solicitud.EstadoCredito = ((Math.random() * (10)) > 5) ? Estado.Aprobada : Estado.Rechazada;
      }else{
  ...
  Validacion(): boolean {
    if (this.solicitudUsuario.usuario.Cedula === '' || this.solicitudUsuario.usuario.Cedula === undefined){
      this.usuarioNoExistente = true;
      this.mensajeError = 'Por favor ingrese una Cedula';
      return false;
    }
    if (this.solicitudUsuario.usuario.Nombre === '' || this.solicitudUsuario.usuario.Nombre === undefined){
      this.usuarioNoExistente = true;
      this.mensajeError = 'Por favor ingrese un Nombre';
      return false;
    }
    if (this.solicitudUsuario.solicitud.ValorSolicitado < environment.ValorMInimoAPrestar
        || this.solicitudUsuario.solicitud.ValorSolicitado > this.montoCapital.Get()
        || this.solicitudUsuario.solicitud.ValorSolicitado === undefined){
      this.usuarioNoExistente = true;
      this.mensajeError = 'Por favor ingrese un monto valido, recuerde que debe esar entre el rango solicitado';
      return false;
    }
    if(this.solicitudUsuario.usuario.Id !== undefined){
          const solicitud = this.solicitudService.GetSolicitudActualXUsuarios(this.solicitudUsuario.usuario.Id);
          if (solicitud.solicitud.EstadoCredito === Estado.Aprobada && !solicitud.solicitud.PagoCredito){
            this.mensajeError = 'El usuario tiene la solicitud "' + solicitud.solicitud.Numero + '" pendiente';
            this.limpiarControles = true;
            return false;
          }
          if (solicitud.solicitud.EstadoCredito === Estado.Rechazada){
            this.mensajeError = 'No se puede realizar una nueva solicitud, ya que el usuario tiene una solicitud anterior en estado '
